...
export const getUserCountry = async () => {
  try {
    ...
    const countryCode = country_code?.toUpperCase() || "NG"; // Default to Nigeria
    const currencyCode = currency?.code || getCurrencyByCountry(countryCode);
    const language = languages?.split(",")[0]?.split("-")[0] || getLanguageByCountry(countryCode);
    console.log("Processed location:", { countryCode, currency: currencyCode, language });
    ...
    try {
      ...
      const { country_code, currency, languages } = response.data;
      const countryCode = country_code?.toUpperCase() || "NG";
      const currencyCode = currency?.code || getCurrencyByCountry(countryCode);
      const language = languages?.split(",")[0]?.split("-")[0] || getLanguageByCountry(countryCode);
      console.log("Processed fallback location:", { countryCode, currency: currencyCode, language });
      return { countryCode, currency: currencyCode, language };
    } catch (fallbackError) {
      console.error("ipapi.co fallback failed:", fallbackError);
      // Default to Nigeria settings
      return { countryCode: "NG", currency: "NGN", language: "ng" };
    }
  }
};