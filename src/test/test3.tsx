...
const FilePreview = React.memo(
  ({ file }: { file: NonNullable<Message["files"]>[0] }) => {
    const displayType =
...
export default function ChatInterface({ token }: ChatInterfaceProps) {
  ...
  const otherParticipantId = useMemo(() => {
    ...
    const chat = activeChatDetails as any;
  ...
  const activeMessages = useMemo(() => {
    ...
    return apiMessages.map((msg: any) => {
      ...
      return {
        _id: msg._id,
        content: msg.content,
        sender: isMe ? "You" : msg.sender?.storeName || "Unknown",
  ...
  useEffect(() => {
    ...
    //   // ignore the server echo of my own messages
    //   if (msg.sender?._id === user.vendor._id) return;
  ...
  const handleFileUpload = useCallback(
    async (
      ...
      if (type === "images" && files.length > 5) {
        showFeedback("Maximum 5 images allowed");
        return;
      }

      if (type === "video" && files[0].size > 100 * 1024 * 1024) {
      ...
      try {
        ...
        if (type === "documents" && docInputRef.current)
    ...
    [activeChat, user.token, token, message, replyingTo, socket, showFeedback]
  );
  ...
  }, [deleteDialog.messageId, token, activeChat, socket]);
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex-1">
                ...
                {isVendorTyping && <TypingIndicator />}
              </div>
              <span className="text-xs text-gray-500">
...
const MessageList = React.memo(
  ({
    ...
    onOpenMedia: (files: Message["files"], index: number) => void;
  }) => (
    <>
      {messages
...
const MessageMeta = React.memo(
  ({
    ...
    >
      ...
      {msg.isEdited && (
        <span
          className={`ml-1 ${msg.isMe ? "text-orange-200" : "text-gray-400"}`}
        >
          (edited)
        </span>
      )}
      ...
      <div className="flex gap-1 ml-2">
        ...
        <button onClick={() => onCopy(msg.content)} className="relative group">
          <Copy className="w-4 h-4" />
        </button>
        {msg.isMe && (
          <>
            <button onClick={() => onEdit(msg._id)} className="relative group">
