...
const TraditionalSoup = () => {
  ...
  useEffect(() => {
    const fetchHairScalpProducts = async () => {
      try {
        ...
        const keywords = [
         "groundnut soup", "egusi soup", "gumbo", "bouillabaisse"
        ];
        ...
        const filtered = allProducts.filter((product: Product) => {
          ...
          return (
            category === "local & traditional foods" &&
  ...
  return (
    <div className="px-8 py-6">
      <h2 className="mb-6 text-2xl font-bold">Traditional Soups & Stews</h2>
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <FaRegSadTear className="mb-4 text-5xl text-gray-300" />
          <h2 className="mb-2 text-2xl font-semibold text-gray-400">
            No Traditional Soups & Stews Product Found
          </h2>
...
export default TraditionalSoup;
