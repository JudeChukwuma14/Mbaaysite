...
export const VendorGoogleButton = () => {
    ...
    const googleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            try {
                ...
                // Use the VENDOR endpoint with the access token
    ...
    return (
        <button
            onClick={() => googleLogin()}
            disabled={isLoading}
            className="flex items-center justify-center w-full p-3 mt-4 font-semibold text-gray-700 transition-all duration-200 bg-white border border-gray-300 rounded-lg shadow-sm hover:shadow-md hover:border-orange-500 hover:ring-2 hover:ring-orange-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
            {isLoading ? (
