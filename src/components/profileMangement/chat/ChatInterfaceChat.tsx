...
const ChatInterfaceChat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUnreadCount = async () => {
      try {
        const data = await getUnreadChatCount(currentUserId);
        console.log("[CHAT] Unread count fetched:", data);
        // Dispatch to redux store or update notification state
        if (data.count !== undefined) {
          dispatch(setTotalUnreadMessages(data.count));
        }
      } catch (error) {
      ...
      try {
        ...
        for (const chat of chatData.chats || []) {
          ...
          if (messageData.success && messageData.messages.length > 0) {
            ...
            console.log("[v0] Chat formatted with unread:", {
              chatName: formattedChat.name,
              unreadCount: formattedChat.unread,
              rawUnreadCount: chat.unreadCount,
            });
        ...
        {isLoadingChats ? (
          ...
          <ChatList
            ...
            onSelectChat={(chatId) => {
              ...
              // Mark chat as read
              if (currentUserId) {
                markChatAsRead(chatId, currentUserId).catch((err) => {
