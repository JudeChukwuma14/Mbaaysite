...
//         filesToSend.map((f) => f.name),
//       )
//       const type = filesToSend[0].type.startsWith("image/")
//         ? "image"
...
//                     <img src={previews[index]} alt={file.name} className="object-cover w-full h-full rounded-lg" />
//                     <Button
//                       variant="ghost"
//                       size="sm"
...
//                   <div className="relative w-24 h-24">
//                     <video src={previews[index]} className="object-cover w-full h-full rounded-lg" />
//                     <Button
//                       variant="ghost"
//                       size="sm"
//                       className="absolute top-0 right-0 w-6 h-6 p-0 bg-black/50 hover:bg-black/70"
...
import { useState, useRef, useEffect } from "react"
import { Send, Paperclip, ImageIcon, Video, Mic, X } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import { Badge } from "@/components/ui/badge"
          ...
          <div className="flex flex-wrap gap-2">
            {attachedFiles.map((file, index) => (
              <div key={index} className="relative">
                {previews[index] && file.type.startsWith("image/") ? (
                  <div className="relative w-24 h-24">
                    <img src={previews[index] || "/placeholder.svg"} alt={file.name} className="object-cover w-full h-full rounded-lg" />
                    <Button
                      variant="ghost"
                      size="sm"
                      className="absolute top-0 right-0 w-6 h-6 p-0 bg-black/50 hover:bg-black/70"
